...
from tkinter import ttk, messagebox, scrolledtext, simpledialog, Toplevel, Menu
import platform
import locale
import hashlib
import traceback
...
class AtendimentoApp:
    def __init__(self, root):
        ...
        # Bindings para desfazer/refazer

        # Bindings para o campo Problema
        self.problema_entry.bind("<Control-z>", lambda e: self.desfazer(self.problema_entry))
        self.problema_entry.bind("<Control-Z>", lambda e: self.desfazer(self.problema_entry))
        self.problema_entry.bind("<Control-y>", lambda e: self.refazer(self.problema_entry))
        self.problema_entry.bind("<Control-Y>", lambda e: self.refazer(self.problema_entry))

        # Bindings para o campo Tarefa
        self.tarefa_entry.bind("<Control-z>", lambda e: self.desfazer(self.tarefa_entry))
        self.tarefa_entry.bind("<Control-Z>", lambda e: self.desfazer(self.tarefa_entry))
        self.tarefa_entry.bind("<Control-y>", lambda e: self.refazer(self.tarefa_entry))
        self.tarefa_entry.bind("<Control-Y>", lambda e: self.refazer(self.tarefa_entry))
        ...
        #Salvar automaticamente após um determinado tempo
        # Variáveis para salvamento automático
        self.ultimo_salvamento = datetime.now()
        self.alteracoes_pendentes = False
        self.timer_salvamento = None
        # Inicia o temporizador de salvamento automático
        self.iniciar_temporizador_salvamento()
    ...
    def criar_menu_contexto(self, event):
        """Menu de contexto unificado para todos os widgets de texto - VERSÃO CORRIGIDA PARA LINUX"""
        widget = event.widget
        ...
        # Verifica se o clique foi em uma palavra com erro ortográfico
        ...
        self._context_menu.add_command(label="Copiar", command=lambda: self.executar_comando_menu(widget, "copy"))
        self._context_menu.add_command(label="Colar", command=lambda: self.executar_comando_menu(widget, "paste"))
        self._context_menu.add_command(label="Recortar", command=lambda: self.executar_comando_menu(widget, "cut"))
        self._context_menu.add_separator()
        self._context_menu.add_command(label="Selecionar Tudo",
                                    command=lambda: self.executar_comando_menu(widget, "select_all"))

        # Adiciona verificação ortográfica se for um campo de texto relevante
        if widget in [self.problema_entry, self.tarefa_entry, self.dados_usuario_text]:
            self._context_menu.add_separator()
            self._context_menu.add_command(label="Verificar Ortografia (F7)",
                                        command=lambda: self.verificar_texto(widget))

        # Exibe o menu com abordagem robusta para Linux
        self._exibir_menu_contexto(event)
    ...
    def configurar_widgets_contexto(self):
        ...
        for widget in widgets:
            ...
            widget.bind("<KeyRelease>", lambda e: None)  # Remove binding anterior

            # Adiciona binding para marcar alterações
            widget.bind("<KeyRelease>", self.marcar_alteracao_e_verificar_ortografia)
    ...
    def criar_menu_superior(self):
        ...
        # Adiciona o menu Temas antes do Sobre
        temas_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Temas", menu=temas_menu)

        # Temas Escuros
        temas_escuros = tk.Menu(temas_menu, tearoff=0)
        temas_menu.add_cascade(label="Temas Escuros", menu=temas_escuros)
        temas_escuros_lista = ["vapor", "cyborg", "solar", "superhero", "darkly"]
        for tema in temas_escuros_lista:
            temas_escuros.add_command(label=tema, command=lambda t=tema: self.aplicar_tema(t))

        # Temas Claro
        temas_claros = tk.Menu(temas_menu, tearoff=0)
        temas_menu.add_cascade(label="Temas Claro", menu=temas_claros)
        temas_claros_lista = ["cerculean", "journal", "morph", "united", "pulse", "yeti",
                            "sandstone", "simplex", "lumen", "minty", "litera", "flatly", "cosmo"]
        for tema in temas_claros_lista:
            temas_claros.add_command(label=tema, command=lambda t=tema: self.aplicar_tema(t))
    ...
    # Exibe uma janela personalizada com informações sobre o software
    def mostrar_sobre(self):
        # Cria a janela
        sobre = tk.Toplevel(self.root)
        sobre.title("Sobre")
        sobre.geometry("400x320")
        sobre.resizable(False, False)
        sobre.transient(self.root)  # Mantém acima da janela principal
        sobre.grab_set()             # Modal

        # Frame principal
        frame = ttk.Frame(sobre, padding=20)
        frame.pack(fill=tk.BOTH, expand=True)

        # Título
        titulo = ttk.Label(frame, text="Gestão de Atendimentos CMZ",
        ...
        info = ttk.Label(frame, text=info_texto, justify=tk.CENTER)
        info.pack(pady=10)

        # Link clicável (use um Label com bind)
        link_label = tk.Label(frame, text="Site do Projeto",
                            fg="blue", cursor="hand2", font=('Helvetica', 10, 'underline'))
        link_label.pack(pady=10)

        # Função para abrir o link no navegador
        def abrir_link(event):
            webbrowser.open("https://github.com/clayzanfolin1/Gestao-de-Atendimentos-CMZ")

        link_label.bind("<Button-1>", abrir_link)

        # Botão Fechar
        ttk.Button(frame, text="Fechar", command=sobre.destroy).pack(pady=10)
    ...
    def criar_janela_clientes(self):
        """Cria a janela de gerenciamento de clientes e usuários com scrollbars funcionais"""
        ...
        self.janela_clientes.geometry("880x390")
        ...
        ttk.Button(btn_frame, text="Renomear Cliente", command=self.renomear_cliente_janela).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="Renomear Usuário", command=self.renomear_usuario_janela).pack(side=tk.LEFT, padx=2)
        ...
        clientes_frame = ttk.Frame(list_frame)
        clientes_frame.grid(row=1, column=0, padx=5, pady=5, sticky=tk.NSEW)

        scroll_clientes = ttk.Scrollbar(clientes_frame)
        ...
        self.lista_clientes = tk.Listbox(
            ...
            exportselection=False  # Importante para manter a seleção
        )
        self.lista_clientes.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scroll_clientes.config(command=self.lista_clientes.yview)
        ...
        usuarios_frame = ttk.Frame(list_frame)
        usuarios_frame.grid(row=1, column=1, padx=5, pady=5, sticky=tk.NSEW)

        scroll_usuarios = ttk.Scrollbar(usuarios_frame)
        ...
        self.cliente_selecionado_label.pack(pady=5)
    ...
    def renomear_cliente_janela(self):
        """Renomeia um cliente e atualiza todos os registros relacionados"""
        selecionado = self.lista_clientes.curselection()
        if not selecionado:
            mostrar_erro(self.janela_clientes, "Selecione um cliente para renomear!")
            return

        cliente_antigo = self.lista_clientes.get(selecionado[0])

        # Solicita o novo nome
        novo_nome = simpledialog.askstring("Renomear Cliente",
                                        f"Novo nome para o cliente '{cliente_antigo}':",
                                        parent=self.janela_clientes)
        ...
        if novo_nome == cliente_antigo:
            return

        if novo_nome in self.clientes:
            mostrar_erro(self.janela_clientes, "Já existe um cliente com este nome!")
            return

        # Confirmação
        confirmacao = messagebox.askyesno(
            "Confirmar Renomeação",
            f"Tem certeza que deseja renomear o cliente '{cliente_antigo}' para '{novo_nome}'?\n"
            "Esta ação atualizará todos os registros do cliente.",
            parent=self.janela_clientes
        )
        ...
        try:
            # 1. Atualiza a lista de clientes
            self.clientes = [novo_nome if c == cliente_antigo else c for c in self.clientes]

            # 2. Atualiza o arquivo clientes.txt CORRIGIDO
            clientes_file = self.base_dir / "clientes.txt"
            if clientes_file.exists():
                with open(clientes_file, "r", encoding="utf-8") as f:
                    linhas = [linha.strip() for linha in f.readlines()]

                # Substitui o nome e mantém uma linha por cliente
                linhas = [novo_nome if linha == cliente_antigo else linha for linha in linhas]

                with open(clientes_file, "w", encoding="utf-8") as f:
                    f.write("\n".join(linhas) + "\n")  # Garante uma linha por cliente e nova linha no final

            # 3. Renomeia o arquivo de usuários
            usuario_antigo = self.usuarios_dir / f"{cliente_antigo}.txt"
            usuario_novo = self.usuarios_dir / f"{novo_nome}.txt"

            if usuario_antigo.exists():
                usuario_antigo.rename(usuario_novo)

            # 4. Atualiza os arquivos de dados do usuário
            for arquivo in self.dados_usuario_dir.glob(f"{cliente_antigo}-*.txt"):
                novo_nome_arquivo = arquivo.name.replace(f"{cliente_antigo}-", f"{novo_nome}-")
                arquivo.rename(self.dados_usuario_dir / novo_nome_arquivo)

            # 5. Atualiza os arquivos de snapshots
            for arquivo in self.dados_usuario_dir.glob(f"{cliente_antigo}-*.old"):
                novo_nome_arquivo = arquivo.name.replace(f"{cliente_antigo}-", f"{novo_nome}-")
                arquivo.rename(self.dados_usuario_dir / novo_nome_arquivo)

            # 6. Atualiza os atendimentos em tmp_atendimentos
            for chave in list(self.tmp_atendimentos.keys()):
                if cliente_antigo in chave:
                    dados = self.tmp_atendimentos.pop(chave)
                    dados["cliente"] = novo_nome
                    nova_chave = chave.replace(cliente_antigo, novo_nome)
                    self.tmp_atendimentos[nova_chave] = dados

            self.salvar_tmp_atendimentos()

            # 7. Atualiza os arquivos de histórico de atendimentos
            for ano_dir in self.atendimentos_dir.iterdir():
                if ano_dir.is_dir():
                    for mes_dir in ano_dir.iterdir():
                        if mes_dir.is_dir():
                            # Atualiza o arquivo do cliente
                            cliente_file = mes_dir / f"{cliente_antigo}.txt"
                            if cliente_file.exists():
                                with open(cliente_file, "r", encoding="utf-8") as f:
                                    conteudo = f.read()

                                novo_conteudo = conteudo.replace(
                                    f"Nome do Cliente: {cliente_antigo}\n",
                                    f"Nome do Cliente: {novo_nome}\n"
                                )
                                ...
                                # Renomeia o arquivo
                                novo_cliente_file = mes_dir / f"{novo_nome}.txt"
                                cliente_file.rename(novo_cliente_file)

                            # Atualiza o arquivo todos.txt
                            todos_file = mes_dir / "todos.txt"
            ...
            # Seleciona o novo nome na lista
            index = self.clientes.index(novo_nome)
            self.lista_clientes.selection_clear(0, tk.END)
            self.lista_clientes.selection_set(index)
            self.lista_clientes.see(index)

            mostrar_sucesso(self.janela_clientes, f"Cliente renomeado de '{cliente_antigo}' para '{novo_nome}' com sucesso!")

        except Exception as e:
            mostrar_erro(self.janela_clientes, f"Erro ao renomear cliente: {str(e)}")
            print(f"Erro detalhado: {traceback.format_exc()}")

    def renomear_usuario_janela(self):
        """Renomeia um usuário e atualiza todos os registros relacionados"""
        # Verifica se há um cliente e usuário selecionados
        if not hasattr(self, 'cliente_selecionado') or not self.cliente_selecionado:
            mostrar_erro(self.janela_clientes, "Selecione um cliente primeiro!")
            return
        ...
        if not selecionado:
            mostrar_erro(self.janela_clientes, "Selecione um usuário para renomear!")
            return

        cliente = self.cliente_selecionado
        usuario_antigo = self.lista_usuarios.get(selecionado[0])

        # Solicita o novo nome do usuário
        novo_nome = simpledialog.askstring("Renomear Usuário",
                                        f"Novo nome para o usuário '{usuario_antigo}':",
                                        parent=self.janela_clientes)
        ...
        if novo_nome == usuario_antigo:
            return

        # Confirmação
        confirmacao = messagebox.askyesno(
            "Confirmar Renomeação",
            f"Tem certeza que deseja renomear o usuário '{usuario_antigo}' para '{novo_nome}'?\n"
            "Esta ação atualizará todos os registros do usuário.",
            parent=self.janela_clientes
        )
        ...
        try:
            # 1. Atualiza o arquivo de usuários do cliente
            usuario_file = self.usuarios_dir / f"{cliente}.txt"
            if usuario_file.exists():
                with open(usuario_file, "r", encoding="utf-8") as f:
                ...
                usuarios = [novo_nome if u == usuario_antigo else u for u in usuarios]

                with open(usuario_file, "w", encoding="utf-8") as f:
                    f.write("\n".join(usuarios) + "\n")

            # 2. Atualiza os arquivos de dados do usuário
            # Renomeia o arquivo principal
            arquivo_antigo = self.dados_usuario_dir / f"{cliente}-{usuario_antigo}.txt".replace("/", "_")
            arquivo_novo = self.dados_usuario_dir / f"{cliente}-{novo_nome}.txt".replace("/", "_")

            if arquivo_antigo.exists():
                arquivo_antigo.rename(arquivo_novo)

            # Renomeia o arquivo de snapshots
            snapshot_antigo = self.dados_usuario_dir / f"{cliente}-{usuario_antigo}.old".replace("/", "_")
            snapshot_novo = self.dados_usuario_dir / f"{cliente}-{novo_nome}.old".replace("/", "_")

            if snapshot_antigo.exists():
                snapshot_antigo.rename(snapshot_novo)

            # 3. Atualiza os atendimentos em tmp_atendimentos
            for chave in list(self.tmp_atendimentos.keys()):
                if f"{cliente} – {usuario_antigo}" == chave:
                    dados = self.tmp_atendimentos.pop(chave)
                    dados["usuario"] = novo_nome
                    nova_chave = f"{cliente} – {novo_nome}"
                    self.tmp_atendimentos[nova_chave] = dados

            self.salvar_tmp_atendimentos()

            # 4. Atualiza os arquivos de histórico de atendimentos
            for ano_dir in self.atendimentos_dir.iterdir():
                if ano_dir.is_dir():
                    for mes_dir in ano_dir.iterdir():
                        if mes_dir.is_dir():
                            # Atualiza o arquivo do cliente
                            cliente_file = mes_dir / f"{cliente}.txt"
                            if cliente_file.exists():
                                with open(cliente_file, "r", encoding="utf-8") as f:
                                    conteudo = f.read()

                                novo_conteudo = conteudo.replace(
                                    f"Usuário: {usuario_antigo}\n",
                                    f"Usuário: {novo_nome}\n"
                                )
            ...
            # Seleciona o novo nome na lista
            usuarios = self.carregar_usuarios_para_cliente(cliente)
            if novo_nome in usuarios:
                index = usuarios.index(novo_nome)
                self.lista_usuarios.selection_clear(0, tk.END)
                self.lista_usuarios.selection_set(index)
                self.lista_usuarios.see(index)

            mostrar_sucesso(self.janela_clientes, f"Usuário renomeado de '{usuario_antigo}' para '{novo_nome}' com sucesso!")

        except Exception as e:
            mostrar_erro(self.janela_clientes, f"Erro ao renomear usuário: {str(e)}")
            print(f"Erro detalhado: {traceback.format_exc()}")
    ...
    def criar_secao_dados_usuario(self, parent):
        ...
        self.dados_usuario_text = scrolledtext.ScrolledText(
            frame, height=10, width=40, undo=True
        )
        self.dados_usuario_text.pack(fill=tk.X, padx=5, pady=5)
        
        # Bindings para o campo Dados do Usuário
        self.dados_usuario_text.bind("<Control-z>", lambda e: self.desfazer(self.dados_usuario_text))
        self.dados_usuario_text.bind("<Control-Z>", lambda e: self.desfazer(self.dados_usuario_text))
        self.dados_usuario_text.bind("<Control-y>", lambda e: self.refazer(self.dados_usuario_text))
        self.dados_usuario_text.bind("<Control-Y>", lambda e: self.refazer(self.dados_usuario_text))
    ...
    # Salvar dados do Usuaário
    def salvar_dados_usuario_agora(self):
        """Salva imediatamente os dados do usuário no arquivo"""
        ...
        if cliente and usuario:
            ...
            # Não marca como alteração pendente pois acabamos de salvar
            self.alteracoes_pendentes = False
    ...
    def retomar_atendimento(self):
        ...
        # Salva os dados do usuário atual antes de trocar
        self.salvar_campos_temporarios()  # Salva problema e tarefa
        if hasattr(self, 'current_atendimento') and self.current_atendimento:
            self.salvar_dados_usuario_agora()

            # LIMPA O CAMPO ANTES DE CARREGAR NOVOS DADOS
    ...
    def criar_secao_clientes(self, parent):
        ...
        self.cliente_combobox.bind('<KeyRelease>', self.autocomplete_cliente)
        ...
        self.usuario_combobox.bind('<KeyRelease>', self.autocomplete_usuario)
    ...
    # autocompletar Cliente e Usuário

    def autocomplete_cliente(self, event):
        """Filtra a lista de clientes enquanto o usuário digita (a partir de 3 caracteres)"""
        texto = self.cliente_var.get().strip()
        if len(texto) >= 3:
            # Filtra clientes que contenham o texto digitado (case insensitive)
            sugestoes = [c for c in self.clientes if texto.lower() in c.lower()]
            self.cliente_combobox['values'] = sugestoes if sugestoes else []
        else:
            # Menos de 3 caracteres: restaura a lista completa
            self.cliente_combobox['values'] = self.clientes
        # NÃO abrimos a lista automaticamente (removido event_generate)

    def autocomplete_usuario(self, event):
        """Filtra a lista de usuários enquanto o usuário digita (a partir de 3 caracteres)"""
        texto = self.usuario_var.get().strip()
        cliente = self.cliente_var.get().strip()
        if cliente:
            # Obtém a lista completa de usuários para o cliente selecionado
            usuarios = self.carregar_usuarios_para_cliente(cliente)
            if len(texto) >= 3:
                sugestoes = [u for u in usuarios if texto.lower() in u.lower()]
                self.usuario_combobox['values'] = sugestoes if sugestoes else []
            else:
                self.usuario_combobox['values'] = usuarios
        else:
            # Sem cliente selecionado: lista vazia
            self.usuario_combobox['values'] = []
        # NÃO abrimos a lista automaticamente (removido event_generate)
    ...
    def criar_secao_atendimento(self, parent):
        ...
        self.problema_entry = scrolledtext.ScrolledText(
            self.atendimento_frame, height=4, width=45, undo=True
        )
    ...
    def iniciar_novo_atendimento(self):
        ...
        # Salva dados do atendimento anterior
        if hasattr(self, 'current_atendimento') and self.current_atendimento:
            self.salvar_dados_usuario_agora()
    ...
    def carregar_historico(self):
        ...
        self.tree.tag_configure('processado', foreground='green')
        self.tree.tag_configure('aberto', foreground='red', font=('Liberation Serif', 11, 'bold'))
        self.tree.tag_configure('presencial', font=('Liberation Serif', 11, 'bold'))
        self.tree.tag_configure('presencial_processado', foreground='green', font=('Liberation Serif', 11, 'bold'))
    ...
    def parse_arquivo_historico(self, conteudo):
        """
        Parseia o conteúdo do arquivo todos.txt, preservando múltiplas linhas
        nos campos Problema e Tarefa.
        """
        atendimentos = []
        ...
        for bloco in blocos:
            ...
            # Divide o bloco em linhas, mantendo as originais (sem strip)
            linhas = bloco.split('\n')
            i = 0
            total = len(linhas)

            # Lista de strings que indicam início de outros campos
            campos_limite = ("Tarefa realizada:", "Histórico de Eventos:", "Tempo Total:", "**********************************")

            while i < total:
                linha = linhas[i].rstrip('\n')  # remove apenas a quebra de linha final

                if linha.startswith("Atendente - Nº do Atendimento:"):
                    partes = linha.split(": ", 1)
                    atend["numero_atendimento"] = partes[1].strip() if len(partes) > 1 else "N/A"

                elif linha.startswith("Tipo:"):
                    partes = linha.split(": ", 1)
                    atend["tipo"] = partes[1].strip() if len(partes) > 1 else "Remoto"

                elif linha.startswith("Nome do Cliente:"):
                    partes = linha.split(": ", 1)
                    atend["cliente"] = partes[1].strip() if len(partes) > 1 else ""

                elif linha.startswith("Usuário:"):
                    partes = linha.split(": ", 1)
                    atend["usuario"] = partes[1].strip() if len(partes) > 1 else ""

                elif linha.startswith("Problema a resolver:"):
                    # Extrai a primeira parte (pode estar vazia)
                    partes = linha.split(": ", 1)
                    problema_linhas = []
                    if len(partes) > 1:
                        problema_linhas.append(partes[1].rstrip())

                    # Avança para a próxima linha
                    i += 1
                    # Coleta todas as linhas seguintes até encontrar um campo limite
                    while i < total:
                        prox_linha = linhas[i].rstrip('\n')
                        # Se a linha atual (sem espaços extras) começar com um campo limite, para
                        if any(prox_linha.lstrip().startswith(campo) for campo in campos_limite):
                            break
                        # Adiciona a linha ao problema (preservando espaços)
                        problema_linhas.append(prox_linha)
                        i += 1
                    # Remove linhas em branco extras no final, mas preserva as internas
                    atend["problema"] = "\n".join(problema_linhas).rstrip('\n')
                    continue  # já incrementamos i no loop interno

                elif linha.startswith("Tarefa realizada:"):
                    partes = linha.split(": ", 1)
                    tarefa_linhas = []
                    if len(partes) > 1:
                        tarefa_linhas.append(partes[1].rstrip())
                    ...
                    while i < total:
                        ...
                        if any(prox_linha.lstrip().startswith(campo) for campo in campos_limite):
                            break
                        tarefa_linhas.append(prox_linha)
                        i += 1
                    atend["tarefa"] = "\n".join(tarefa_linhas).rstrip('\n')
                    continue

                elif linha.startswith("Histórico de Eventos:"):
                    # Processa os eventos a partir daqui
                    i += 1
                    eventos_linhas = []
                    while i < total:
                        prox_linha = linhas[i].rstrip('\n')
                        if not prox_linha.strip():
                            i += 1
                            continue
                        # Se encontrar "Tempo Total:" ou separador, para
                        if prox_linha.startswith("Tempo Total:") or prox_linha.startswith("**********************************"):
                            break
                        eventos_linhas.append(prox_linha)
                        i += 1

                    # Converte as linhas de eventos para a estrutura de dados
                    for ev_line in eventos_linhas:
                        ev_line_lower = ev_line.lower()
                        if any(ev_line_lower.startswith(t) for t in ["início:", "inicio:", "pausa:", "retomada:", "fim:"]):
                            partes = ev_line.split(":", 1)
                            tipo = partes[0].strip().lower()
                            if tipo == "início":
                                tipo = "inicio"
                            data_hora = partes[1].strip().split()
                            if len(data_hora) == 2:
                                try:
                                    data = datetime.strptime(data_hora[0], "%d/%m/%Y").date()
                                    hora = datetime.strptime(data_hora[1], "%H:%M").time()
                                    atend["eventos"].append({"tipo": tipo, "data": data, "hora": hora})
                                except ValueError:
                                    continue
                    # Após os eventos, a linha atual é "Tempo Total:" – será tratada na próxima iteração
                    continue

                elif linha.startswith("Tempo Total:"):
                    # Extrai o tempo, mas o cálculo será refeito depois se necessário
                    # Não precisamos armazenar aqui, pois recalculamos com os eventos
                    pass

                i += 1

            # Calcula o tempo total com base nos eventos (se não veio do arquivo)
    ...
    def visualizar_detalhes(self, event):
        ...
        detalhes_window.geometry("950x720")
        ...
        # Bindings para desfazer/refazer
        problema_entry.bind("<Control-z>", lambda e: self.desfazer(problema_entry))
        problema_entry.bind("<Control-Z>", lambda e: self.desfazer(problema_entry))
        problema_entry.bind("<Control-y>", lambda e: self.refazer(problema_entry))
        problema_entry.bind("<Control-Y>", lambda e: self.refazer(problema_entry))
        ...
        # Bindings para desfazer/refazer
        tarefa_entry.bind("<Control-z>", lambda e: self.desfazer(tarefa_entry))
        tarefa_entry.bind("<Control-Z>", lambda e: self.desfazer(tarefa_entry))
        tarefa_entry.bind("<Control-y>", lambda e: self.refazer(tarefa_entry))
        tarefa_entry.bind("<Control-Y>", lambda e: self.refazer(tarefa_entry))
    ...
    def criar_janela_anotacoes(self):
        ...
        try:
            ...
            self.janela_anotacoes.geometry("820x620")
    ...
    def ao_fechar_janela(self):
        """Salva tudo antes de fechar a aplicação"""
        # Salva os dados pendentes
        if self.alteracoes_pendentes:
            self.salvar_tudo_automaticamente()

        # Para o temporizador
        self.parar_temporizador_salvamento()
    ...
    def configurar_estilo_menu_linux(self):
        ...
        if platform.system() == "Linux":
            # Configurações apenas para widgets ttk (não usar para Menu)
            style = ttk.Style()
            ...
            # Para menus de contexto (widgets Menu tradicionais), usamos option_add
            # Isso configura os menus de contexto globalmente
            self.root.option_add('*Menu*background', 'white')
            self.root.option_add('*Menu*foreground', 'black')
            self.root.option_add('*Menu*activeBackground', 'lightblue')
            self.root.option_add('*Menu*activeForeground', 'black')
            self.root.option_add('*Menu*selectColor', 'lightblue')
    ...
    def criar_janela_busca(self):
        ...
        self.busca_tree.tag_configure('nao_processado', foreground='blue')
        self.busca_tree.tag_configure('cancelado', foreground='orange')
        self.busca_tree.tag_configure('presencial', font=('Liberation Serif', 11, 'bold'))
        self.busca_tree.tag_configure('presencial_processado', foreground='green', font=('Liberation Serif', 11, 'bold'))
    ...
    def verificar_palavra_na_posicao(self, widget, index):
        ...
        try:
            ...
            # Limpa a palavra removendo pontuação antes de verificar
            palavra_limpa = re.sub(r'[^\wÀàÁáÂâÃãÄäÈèÉéÊêËëÌìÍíÎîÏïÒòÓóÔôÕõÖöÙùÚúÛûÜüÇç]', '', palavra)

            # Verifica se é uma palavra válida para verificação e não está na lista de ignoradas
            if (self.is_palavra_valida(palavra) and
                palavra_limpa.lower() not in self.palavras_ignoradas and
                palavra_limpa.lower() not in self.dicionario):
    ...
    def marcar_palavra_errada(self, widget, palavra):
        ...
        # Verifica se é uma palavra válida para verificação e não está na lista de ignoradas
        if (not self.is_palavra_valida(palavra) or
            palavra_limpa.lower() in self.palavras_ignoradas or
            palavra_limpa.lower() in self.dicionario):
            return

        start_idx = "1.0"
        while True:
            # Busca a palavra exata (com pontuação) no texto
            start_idx = widget.search(re.escape(palavra), start_idx, stopindex=tk.END, regexp=False)
            if not start_idx:
                break
    ...
    #método auxiliar para limpar pontuação
    def limpar_pontuacao(self, texto):
        """Remove toda a pontuação de uma string"""
        return re.sub(r'[^\wÀàÁáÂâÃãÄäÈèÉéÊêËëÌìÍíÎîÏïÒòÓóÔôÕõÖöÙùÚúÛûÜüÇç]', '', texto)

    def obter_sugestoes(self, palavra):
        """Retorna sugestões de correção para a palavra de forma otimizada"""
        # Limpa a pontuação antes de buscar sugestões
        palavra_limpa = self.limpar_pontuacao(palavra)
        ...
        prefixo = palavra_lower[:3] if len(palavra_lower) >= 3 else palavra_lower
    ...
    def executar_comando_menu(self, widget, comando):
        """Executa comandos do menu e fecha o menu"""
        if comando == "copy":
        ...
        elif comando == "select_all":
            widget.tag_add(tk.SEL, "1.0", tk.END)
            widget.mark_set(tk.INSERT, "1.0")
            widget.see(tk.INSERT)

        # Fecha o menu após executar o comando
        if hasattr(self, '_context_menu') and self._context_menu.winfo_exists():
            self._context_menu.destroy()

    def _exibir_menu_contexto(self, event):
        """Exibe o menu de contexto com gerenciamento robusto de eventos"""
        try:
            # Exibe o menu
            self._context_menu.post(event.x_root, event.y_root)

            # Configura o binding para fechar o menu quando clicar fora
            self._context_menu_bind_id = self.root.bind("<ButtonPress>", self._fechar_menu_ao_clicar_fora, add="+")

            # Fecha ao pressionar Escape
            self.root.bind("<Escape>", self._fechar_menu, add="+")

            # Remove bindings quando o menu é fechado
            self._context_menu.bind("<Unmap>", lambda e: self._limpar_bindings_menu())

        except Exception as e:
            print(f"Erro ao exibir menu de contexto: {e}")

    def _fechar_menu_ao_clicar_fora(self, event):
        """Fecha o menu quando o usuário clica fora dele"""
        if hasattr(self, '_context_menu') and self._context_menu.winfo_exists():
            # Verifica se o clique foi fora do menu
            menu_x = self._context_menu.winfo_rootx()
            menu_y = self._context_menu.winfo_rooty()
            menu_width = self._context_menu.winfo_width()
            menu_height = self._context_menu.winfo_height()
    ...
    def _fechar_menu(self, event=None):
        ...
        if hasattr(self, '_context_menu') and self._context_menu.winfo_exists():
            self._context_menu.unpost()
            self._context_menu.destroy()
            self._limpar_bindings_menu()

    def _limpar_bindings_menu(self):
        """Remove os bindings do menu de contexto"""
        try:
    ...
    def criar_menu_correcao(self, event, texto_widget):
        """Cria menu específico para correção ortográfica - VERSÃO CORRIGIDA"""
        if hasattr(self, '_context_menu') and self._context_menu.winfo_exists():
            self._context_menu.destroy()
        ...
        if sugestoes:
            ...
            for sugestao in sugestoes[:10]:
                menu.add_command(
                    label=sugestao,
                    command=lambda s=sugestao: self.corrigir_palavra_menu(texto_widget, s))
        else:
            menu.add_command(label="Nenhuma sugestão disponível", state=tk.DISABLED)

        menu.add_separator()
        menu.add_command(label="Ignorar", command=lambda: self.ignorar_erro_menu(texto_widget))
        menu.add_command(label="Adicionar ao dicionário",
                        command=lambda: self.adicionar_ao_dicionario_menu(self.palavra_errada.lower()))

        # Exibe o menu
        self._exibir_menu_contexto(event)

    def corrigir_palavra_menu(self, texto_widget, sugestao):
        """Corrige a palavra e fecha o menu"""
        self.corrigir_palavra(texto_widget, sugestao)
        self._fechar_menu()

    def ignorar_erro_menu(self, texto_widget):
        """Ignora o erro e fecha o menu"""
        self.ignorar_erro(texto_widget)
        self._fechar_menu()

    def adicionar_ao_dicionario_menu(self, palavra):
        """Adiciona ao dicionário e fecha o menu"""
        self.adicionar_ao_dicionario(palavra)
        self._fechar_menu()
    ...
    def is_palavra_valida(self, texto):
        ...
        # Remove TODA a pontuação antes de verificar, não apenas no final
        palavra_limpa = re.sub(r'[^\wÀàÁáÂâÃãÄäÈèÉéÊêËëÌìÍíÎîÏïÒòÓóÔôÕõÖöÙùÚúÛûÜüÇç]', '', texto)

        # Ignora números puros e strings vazias após limpeza
        if not palavra_limpa or palavra_limpa.isdigit():
            return False

        # Verifica se tem pelo menos uma letra
        return any(c.isalpha() for c in palavra_limpa)

    #Sistema de salvamento automático

    # Adicione este método para iniciar o temporizador de salvamento
    def iniciar_temporizador_salvamento(self):
        """Inicia o temporizador para salvamento automático a cada 15 segundos"""
        if self.timer_salvamento:
            self.root.after_cancel(self.timer_salvamento)

        self.timer_salvamento = self.root.after(15000, self.verificar_salvamento_automatico)

    # Adicione este método para verificar se precisa salvar
    def verificar_salvamento_automatico(self):
        """Verifica se há alterações pendentes e salva a cada 15 segundos"""
        if self.alteracoes_pendentes:
            self.salvar_tudo_automaticamente()
    ...
    # Adicione este método para salvar tudo automaticamente
    def salvar_tudo_automaticamente(self):
        """Salva todos os campos automaticamente"""
        try:
            # Salva os campos de problema e tarefa
            self.salvar_campos_temporarios()
            ...
            print("Dados salvos automaticamente às", self.ultimo_salvamento.strftime("%H:%M:%S"))

        except Exception as e:
            print(f"Erro no salvamento automático: {e}")

    # Adicione este método para marcar alterações pendentes
    def marcar_alteracao_pendente(self, event=None):
        """Marca que há alterações pendentes para salvar"""
        self.alteracoes_pendentes = True

    # Adicione este método combinado
    def marcar_alteracao_e_verificar_ortografia(self, event):
        """Marca alteração pendente e verifica ortografia"""
        self.marcar_alteracao_pendente()
        self.verificar_palavra_na_posicao(event.widget, "insert")

        # Inicia/reinicia o temporizador de salvamento
        self.iniciar_temporizador_salvamento()

    # Adicione este método para parar o temporizador quando necessário
    def parar_temporizador_salvamento(self):
        """Para o temporizador de salvamento automático"""
        if self.timer_salvamento:
            self.root.after_cancel(self.timer_salvamento)
            self.timer_salvamento = None

    #Criação menu de tema

    def aplicar_tema(self, tema):
        """Aplica um novo tema ao aplicativo após confirmação"""
        confirmacao = messagebox.askyesno(
            "Confirmar Tema",
            f"Deseja aplicar o tema '{tema}'?",
            parent=self.root
        )
        ...
        if confirmacao:
            try:
                ...
                with open(self.config_dir / "tema.txt", "w", encoding="utf-8") as f:
                    f.write(tema)

                mostrar_sucesso(self.root, f"Tema '{tema}' aplicado com sucesso!")
            except Exception as e:
                mostrar_erro(self.root, f"Erro ao aplicar tema: {str(e)}")

    def carregar_tema_salvo(self):
        """Carrega o tema salvo nas configurações"""
        tema_path = self.config_dir / "tema.txt"
        try:
            if tema_path.exists():
    ...
    # métodos desfazer e refazer  
    def desfazer(self, widget):
        """Executa a ação de desfazer no widget (Ctrl+Z) e interrompe a propagação do evento"""
        try:
            widget.edit_undo()
        except tk.TclError:
            # Não há mais ações para desfazer
            pass
        return "break"   # ← IMPEDE PROPAGAÇÃO

    def refazer(self, widget):
        """Executa a ação de refazer no widget (Ctrl+Y) e interrompe a propagação do evento"""
        try:
            widget.edit_redo()
        except tk.TclError:
